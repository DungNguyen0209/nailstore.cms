<template>
    <div v-if="isNumberEnterStep" class="flex flex-col items-center justify-center h-screen bg-pink-100 p-4 md:p-8">
      ...
      <div class="flex flex-col md:flex-row items-center w-full gap-8">
        <!-- Points Badge -->
         <div class="flex justify-center w-6/12">
           <div class="bg-gray-800 text-white p-6 md:p-8 rounded-lg flex flex-col items-center shadow-lg transform transition duration-200 hover:-translate-y-1 w-full md:w-2/3 md:h-64">
             <h1 class="font-bold points">{{ points }}</h1>
             <p class="text-lg md:text-xl mt-2 opacity-80">Punkte</p>
             <p class="text-sm md:text-lg mt-1 opacity-80">Entsprechen {{ (points / 40).toFixed(2) }}€</p>
           </div>
         </div>
      ...
      <footer class="mt-6 text-sm text-gray-600 text-center max-w-xs">
        <label class="flex items-center space-x-2">
          ...
          />
          <span>Mit dem Check-in erklären Sie sich mit unserer Datenschutzrichtlinie und den Geschäftsbedingungen des Geschäfts einverstanden.</span>
        </label>
      </footer>
    </div>
    <div v-else class="background-container">
        ...
        <div class="space-y-8 mb-8 ">
          ...
          <FloatLabel variant="in">
              <InputText fluid borderless  id="on_label" class="font-arial text-sm" v-model="account.email" />
...
.custom-button {
  ...
  width: 5rem !important; /* 16 * 0.25rem */
  height: 5rem !important; /* 16 * 0.25rem */
  color: #102c57 !important;
  border-color: #102c57 !important;
  font-size: 1.5rem !important; /* 2 * 0.75rem */
}
...
.custom-button:active {
  background-color: #ec48ad !important; /* pink-400 */
  border-color: #ec48ad !important; /* pink-400 */
}

.thin-icon {
  font-size: 2rem !important;
  font-weight: 300 !important; /* Adjust the weight to make it thinner */
}
