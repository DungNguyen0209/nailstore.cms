...
<template>
  <LayoutAuthenticated>
    <SectionMain class="h-full flex flex-col relative">
      <SectionTitleLineWithButton :icon="mdiCreditCardCheck" title="Credit Point" :buttonIcon="mdiPlus" main>
      ...
      <div id="main"  class="h-full w-full">
        <div v-for="tier in tiers" :key="tier.id" class="w-full">
              <div class="mb-4 w-full flex flex-col sm:flex-row">
                <Card style="overflow: hidden" class="w-full sm:w-4/5 content-center hover:shadow-lg hover:transition-all duration-500 cursor-pointer">
                  <template #header>
                    ...
                    <div v-if="tier.name !== defaultTier.Basic" class="flex flex-col items-center">
                      <div class="w-full flex flex-col gap-2 sm:flex-row">
                        <div class="flex flex-col w-full sm:w-1/2 sm:justify-center sm:ml-2">
                          ...
                          <InputText id="visittime" v-model="tier.requiredVisit" class="w-2/3" autocomplete="off" />
                        </div>
                  ...
                  <template #footer>
                    <div class="w-full flex flex-row">
                      ...
                      <div class="w-3/4 flex flex-row justify-between">
                        <div class="w-6/12 flex flex-col h-full">
                          <label for="credit" class="font-semibold h-full item align-bottom mr-2">Point</label>
                          <InputNumber v-model="tier.creditPointSetting.requiredPoints" id="credit"/>                        
                        </div>
                        <Button class="w-1/12 h-1/2 top-8 font-extrabold" icon="pi pi-equals" text />
                        <div class="w-5/12 flex flex-col h-full">
                          <label for="credit" class="font-semibold h-full item align-bottom mr-2">Discount</label>
                          <InputGroup>
                              <InputGroupAddon>$</InputGroupAddon>
                              <InputNumber v-model="tier.creditPointSetting.price" placeholder="Price" />
                              <InputGroupAddon>.00</InputGroupAddon>
                          </InputGroup>
                        </div>
                      </div>  
                ...
                <div class="flex flex-row gap-3 mt-2 w-full sm:w-1/5 items-center justify-center">
                  ...
                  <Button v-if="tier.name !== defaultTier.Basic" @click="() => removeTier(tier)" icon="pi pi-trash" aria-label="Filter" />
                </div>
              </div>
        </div>
        <div class="mt-2 ">
