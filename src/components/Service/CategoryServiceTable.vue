...
<template>
  <DataTable v-model:selection="selectedService" :value="categories" dataKey="id" :paginator="true" lazy scrollable
    :scrollHeight="'calc(48vh)'" :totalRecords="totalRecords" :rows="pageSize" :loading="isloading" class="w-full"
    @page="onPageChange"
    ...
    <Column header-class="sticky! top-0! z-10000" :exportable="false" class="w-0.5/6">
      <template #body="slotProps">
        <div class="flex flex-row h-full gap-2 content-center align-middle">
          <Button icon="pi pi-pencil" outlined rounded class="mb-2" @click="editCategory(slotProps.data)" />
